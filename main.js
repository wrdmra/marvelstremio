...
function toMeta(item, forcedType) {
  ...
  const meta = {
    ...
    description:
      cached.description || item.description || "Keine Beschreibung verfügbar.",
...
function buildManifest(origin) {
  return {
    ...
    version: "1.0.0",
    name: "Marvel Katalog DE",
    description:
      "Marvel-Serien, Marvel-Filme und die MCU-Filme in chronologischer Reihenfolge.",
...
if (isMainModule) {
  createServer().listen(PORT, "0.0.0.0", () => {
    console.log(`Marvel Katalog DE läuft auf http://localhost:${PORT}`);
  });
}
